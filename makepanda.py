...
# agressively unoptimize, for msvc compiler only at moment
FORCEAGRESSIVENOOPTIMIZE = 0
...
def packageInfo():
  ...
  Misc libraries:
    ...
    FRACTALSPLINE FractalSpline
              http://sf.net/projects/fractalspline
              Advanced parametric prims
...
def usage(problem):
    ...
    print "  --advancedoptions (print advanced options)"
    print ""
...
def advancedoptions():
   ...
   print ""
   print "--vcpptoolkit X       (path to Microsoft Visual C++ Toolkit 2003 directory)"
   print "--msplatformsdk X     (path to Microsoft Platform SDK directory)"
   print "--directx-sdk X       (path to Microsoft DirectX SDK directory)"
   print "--no-resource-compiler(no resource compiler; use with Microsoft C++ Toolkit)"
   print "--no-interrogate      (dont run interrogate; use to speed up building if you dont need Python interface)"
   print "--no-dx8              (dont build DirectX 8 pipeline)"
   print "--fractalspline X     (path to FractalSpline library)"
   sys.exit(1)

def parseopts(args):
    global PREFIX,COMPILER,OPTIMIZE,OMIT,THIRDPARTY,INSTALLER,PPGAME
    global COPYEXTRAS,VERSION,COMPRESSOR,DIRECTXSDK,VERBOSE, VISUALCPPTOOLKIT2003
    global MSPLATFORMSDK, COMPILERES, RUNINTERROGATE, HAVEDIRECTX8
    global FORCEAGRESSIVENOOPTIMIZE, FRACTALSPLINE
    longopts = [
        "help","advancedoptions","package-info","prefix=","compiler=","directx-sdk=","vcpptoolkit2003=","thirdparty=",
        "optimize=","everything","nothing","installer","ppgame=","quiet","verbose","msplatformsdk=",
        "version=","lzma","no-resource-compiler", "no-interrogate", "no-dx8", "force-agressive-no-optimize",
        "fractalspline=" ]
    anything = 0
    ...
    try:
        ...
        for option,value in opts:
            ...
            elif (option=="--advancedoptions"): advancedoptions()
            elif (option=="--package-info"): raise "package-info"
            elif (option=="--prefix"): PREFIX=value
            elif (option=="--compiler"): COMPILER=value
            elif (option=="--no-resource-compiler"):
                COMPILERES = 0
                print "-Resource compiler"
            elif (option=="--no-resource-compiler"):
                COMPILERES = 0
                print "-Resource compiler"
            elif (option=="--directx-sdk"):
                DIRECTXSDK=value
                print "+Directxsdk: " + DIRECTXSDK
            elif (option=="--vcpptoolkit2003"):
                VISUALCPPTOOLKIT2003=value
                print "+Visual C++ Toolkit 2003: " + VISUALCPPTOOLKIT2003
            elif (option=="--msplatformsdk"):
                MSPLATFORMSDK=value
                print "+Microsoft Platform SDK: " + MSPLATFORMSDK
            elif (option=="--fractalspline"):
                FRACTALSPLINE=value
                print "+FractalSpline: " + FRACTALSPLINE
            elif (option=="--no-interrogate"):
                print "-Interrogate"
                RUNINTERROGATE = 0
            elif (option=="--no-dx8"):
                print "-DirectX8"
                HAVEDIRECTX8 = 0
            elif (option=="--force-agressive-no-optimize"):
                print "+force agressive no optimize"
                FORCEAGRESSIVENOOPTIMIZE = 1
...
if (COMPILER == "MSVC7"):
    if VISUALCPPTOOLKIT2003 is None:
        ...
        if MSPLATFORMSDK is None:
            print ""
            print "Please rerun makepanda.py, adding the path to the Microsoft Platform SDK"
            print "You can use option --msplatformsdk X  to do this"
            sys.exit(1)
        vcdir = VISUALCPPTOOLKIT2003
        old_env_path    = ""
        old_env_include = ""
        old_env_lib     = ""
        if (os.environ.has_key("PATH")):    old_env_path    = os.environ["PATH"]
        if (os.environ.has_key("INCLUDE")): old_env_include = os.environ["INCLUDE"]
        if (os.environ.has_key("LIB")):     old_env_lib     = os.environ["LIB"]
        os.environ["PATH"] = VISUALCPPTOOLKIT2003 + "\\bin;" + old_env_path
        os.environ["INCLUDE"] = MSPLATFORMSDK + "\\INCLUDE;" + VISUALCPPTOOLKIT2003 + "\\include;" + DIRECTXSDK + "\\include;" + old_env_include
        os.environ["LIB"] = MSPLATFORMSDK + "\\LIB;" + VISUALCPPTOOLKIT2003 + "\\lib;" + DIRECTXSDK + "\\lib;" + old_env_lib
        sys.stdout.flush()        
...
########################################################################
##
## Checks whether directory directoryname exists
## Prints +[directoryname] if it does, else prints a warning
## for use in printStatus
##
## Created by Hugh Perkins May 2005
##
########################################################################
def checkdir( directoryname ):
    ...
    if os.path.exists( directoryname ): 
        ...
        print "Warning: " + directoryname + " not found"
...
def printStatus(header,warnings):
    ...
    if VERBOSE >= -2:
        ...
        if not MSPLATFORMSDK is None:
           print "Makepanda: Microsoft Platform SDK dir:",MSPLATFORMSDK
        ...
        directories = ["pandatool", "doc", "dtool", "panda", "pandaapp",
            "pandatool", "direct", "makepanda", "thirdparty", "SceneEditor",
            "models", "samples", "dmodels" ]
...
## getheaderfilesin
##
## returns a list containing all .h files in directorypath
## directorypath can be a relative path
##
## Created by Hugh Perkins, May 2005
##
########################################################################

def getheaderfilesin( directorypath ):
    import os
    headerfiles = []
...
def CompileC(obj=0,src=0,ipath=[],opts=[]):
    ...
    if (COMPILER=="MSVC7"):
        ...
        if (older(wobj, dep)):
            ...
            if( not FRACTALSPLINE is None ):
                cmd = cmd + ' /I"' + FRACTALSPLINE + '" /I"F:\dev\mathgl-pp-0.5.0\src\libmathglpp"'
            ...
            # FORCEAGRESSIVENOOPTIMIZE added by Hugh Perkins May 2005 to speed up dev builds some
            if not FORCEAGRESSIVENOOPTIMIZE:
...
def CompileLIB(lib=0, obj=[], opts=[]):
    ...
    if (COMPILER=="MSVC7"):
        ...
        if (older(wlib, wobj)):
            # modified to use link /lib instead of lib (VC toolkit doesnt have lib), Hugh Perkins May 2005
...
def CompileLink(dll=0, obj=[], opts=[], xdep=[]):
    ...
    if (COMPILER=="MSVC7"):
        ...
        if (older(dll, wobj+xdep)):
            ...
            # Added by Hugh Perkins May 2005, to speed up dev builds in absence of debug runtime libraries
            if FORCEAGRESSIVENOOPTIMIZE:
               cmd = 'link.exe /nologo /NODEFAULTLIB:LIBCI.LIB /DEBUG /NODEFAULTLIB:MSVCRTD.LIB /incremental /stack:4194304 '
            ...
            for x in wobj: cmd = cmd + ' ' + x
            if (dll[-4:]==".exe" and COMPILERES ): cmd = cmd + ' ' + PREFIX + '/tmp/pandaIcon.res'
            ...
            if (opts.count("FRACTALSPLINE")):
                cmd = cmd + ' /LIBPATH:"' + FRACTALSPLINE + '" libfractalspline.lib'
    ...
    if (COMPILER=="LINUXA"):
        ...
        if (older(wdll, wobj+xdep)):
            ...
            if (opts.count("FRACTALSPLINE")):  cmd = cmd + " -lfractalspline"
            oscmd(cmd)
            updatefiledate(wdll)
...
if (sys.platform == "win32" and COMPILERES ):
...
# I'd vote for copying all headers?  In any case, the hardcoded lines are currently
# broken following the migration to qpGeom etc.
# I'm going to assume Python 2.2 here, so we'll use os.path.walk, but there
# is an easier to use implementation in Python 2.3+ I think(?)
# - Hugh Perkins, May 2005
...
def WalkCallback( arg, dirname, names):
    for filename in names:
        ...
        if extension == ".h" or extension == ".I" or extension == ".T":
            ...
            dirpathhead, dirpathtail = os.path.split( dirname )
            # print dirpathtail
            
            # Handle any exceptions.  We dont want to copy from these directories.  - Hugh
            if dirpathtail == "parser-inc":
                return
            if dirpathtail == "cppparser":
                return
                
            CopyFile(PREFIX+'/include/', fullfilepath )
    
import os.path
os.path.walk( 'dtool/src', WalkCallback, None )
os.path.walk( 'panda/src', WalkCallback, None )

if os.path.exists( 'pandatool/src' ):
    os.path.walk( 'pandatool/src', WalkCallback, None )
    
if os.path.exists( 'pandaapp/src' ):
    os.path.walk( 'pandaapp/src', WalkCallback, None )
    
if os.path.exists( 'direct/src' ):
    os.path.walk( 'direct/src', WalkCallback, None )

# A few tweaks:
CopyFile(PREFIX+'/include/','dtool/src/dtoolutil/vector_src.cxx')
    
# if 0 basically disables this section.  Hugh Perkins, May 2005
if 0:
...
OBJECTS=   [ 'pandaexpress_pandaexpress.obj',
...
if RUNINTERROGATE:
    OBJECTS.append( 'libpandaexpress_module.obj' )
CompileLink(opts=['ADVAPI', 'WINSOCK2', 'NSPR', 'SSL', 'ZLIB'], dll='libpandaexpress.dll', obj=OBJECTS )
...
# migrated to just include every .h file in the directory - Hugh Perkins, May 2005
if RUNINTERROGATE:
    files=getheaderfilesin( "panda/src/gobj" )
    files.append( 'gobj_composite1.cxx' )
    files.append( 'gobj_composite2.cxx' )
    Interrogate(ipath=IPATH, opts=OPTS, outd='libgobj.in', outc='libgobj_igate.cxx',
                src='panda/src/gobj',  module='panda', library='libgobj',
                files=files )
                # [
                # 'boundedObject.h', 'config_gobj.h', 'drawable.h', 'geom.h', 'geomContext.h',
                # 'geomLine.h', 'geomLinestrip.h', 'geomPoint.h', 'geomPolygon.h', 'geomQuad.h',
                # 'geomSphere.h', 'geomSprite.h', 'geomTri.h', 'geomTrifan.h', 'geomTristrip.h',
                # 'qpgeom.h', 'qpgeomCacheEntry.h', 'qpgeomEnums.h', 'qpgeomLines.h', 'qpgeomLinestrips.h',
                # 'qpgeomMunger.h', 'qpgeomPoints.h', 'qpgeomPrimitive.h', 'qpgeomTriangles.h', 
                # 'qpgeomTrifans.h', 'qpgeomTristrips.h', 'qpgeomVertexAnimationSpec.h',
                # 'qpgeomVertexArrayData.h', 'qpgeomVertexArrayFormat.h', 
                # 'material.h', 'materialPool.h', 'matrixLens.h', 'orthographicLens.h',
                # 'perspectiveLens.h', 'preparedGraphicsObjects.h', 'lens.h', 'savedContext.h',
                # 'texture.h', 'textureContext.h', 'texturePool.h', 'textureStage.h',
...
# composites 3 and 4 added by Hugh Perkins May 2005
...
# migrated to use getheaderfilesin, Hugh Perkins May 2005
if RUNINTERROGATE:
    files = getheaderfilesin("panda/src/pgraph")
    files.append( 'pgraph_composite1.cxx')
    files.append( 'pgraph_composite2.cxx')
    files.append( 'pgraph_composite3.cxx')
    files.append( 'pgraph_composite4.cxx')
    Interrogate(ipath=IPATH, opts=OPTS, outd='libpgraph.in', outc='libpgraph_igate.cxx',
                src='panda/src/pgraph',  module='panda', library='libpgraph', files=files )
                # 'accumulatedAttribs.h', 'alphaTestAttrib.h', 'ambientLight.h', 'auxSceneData.h', 
                # 'bamFile.h', 'billboardEffect.h', 'binCullHandler.h', 'camera.h', 
                #'clipPlaneAttrib.h', 'colorAttrib.h', 'colorBlendAttrib.h', 'colorScaleAttrib.h', 
                #'colorWriteAttrib.h', 'compassEffect.h', 'config_pgraph.h', 'cullBin.h', 
                #'cullBinAttrib.h', 'cullBinBackToFront.h', 'cullBinFixed.h', 'cullBinFrontToBack.h',
                # 'cullBinManager.h', 'cullBinUnsorted.h', 'cullFaceAttrib.h', 'cullHandler.h', 
                #'cullResult.h', 'cullTraverser.h', 'cullTraverserData.h', 'cullableObject.h', 
                #'decalEffect.h', 'depthOffsetAttrib.h', 'depthTestAttrib.h', 'depthWriteAttrib.h',
                # 'directionalLight.h', 'drawCullHandler.h', 'fadeLodNode.h', 'fadeLodNodeData.h', 
                #'findApproxLevelEntry.h', 'findApproxPath.h', 'fog.h', 'fogAttrib.h', 'geomNode.h',
                # 'geomTransformer.h', 'lensNode.h', 'light.h', 'lightAttrib.h', 'lightLensNode.h',
                # 'lightNode.h', 'loader.h', 'loaderFileType.h', 'loaderFileTypeBam.h',
                # 'loaderFileTypeRegistry.h', 'lodNode.h', 'materialAttrib.h', 'modelNode.h', 
                #'modelPool.h', 'modelRoot.h', 'nodePath.h', 'nodePath.cxx', 'nodePathCollection.h',
                # 'nodePathComponent.h', 'nodePathLerps.h', 'pandaNode.h', 'planeNode.h', 
                #'pointLight.h', 'polylightNode.h', 'polylightEffect.h', 'portalNode.h',
                # 'portalClipper.h', 'renderAttrib.h', 'renderEffect.h', 'renderEffects.h',
                # 'renderModeAttrib.h', 'renderState.h', 'rescaleNormalAttrib.h',
                # 'sceneGraphAnalyzer.h', 'sceneGraphReducer.h', 'sceneSetup.h',
                # 'selectiveChildNode.h', 'sequenceNode.h', 'showBoundsEffect.h', 'spotlight.h',
                # 'switchNode.h', 'texMatrixAttrib.h', 'texProjectorEffect.h', 'textureApplyAttrib.h',
                # 'textureAttrib.h', 'texGenAttrib.h', 'textureCollection.h',
                # 'textureStageCollection.h', 'transformState.h', 'transparencyAttrib.h', 
                #'weakNodePath.h', 'workingNodePath.h', 'pgraph_composite1.cxx', 
                #'pgraph_composite2.cxx'])
...
INFILES=['librecorder.in', 'libpgraph.in', 'libgrutil.in', 'libchan.in', 'libpstatclient.in',
         ...
         'libpgui.in']
OBJFILES=['panda_panda.obj', 'recorder_composite1.obj',
          'recorder_composite2.obj', 
          'pgraph_nodePath.obj', 'pgraph_composite1.obj', 'pgraph_composite2.obj',
          # composites 3 and 4 added by Hugh Perkins May 2005
          'pgraph_composite3.obj','pgraph_composite4.obj',
          'grutil_multitexReducer.obj', 'grutil_composite1.obj',
          'chan_composite1.obj', 'chan_composite2.obj', 'pstatclient_composite1.obj',
          'pstatclient_composite2.obj', 'char_composite1.obj',
          'char_composite2.obj', 'collide_composite1.obj', 'collide_composite2.obj',
          'device_composite1.obj', 'device_composite2.obj',
          'dgraph_composite1.obj', 'dgraph_composite2.obj', 'display_composite1.obj',
          'display_composite2.obj', 'event_composite1.obj', 
          'gobj_composite1.obj', 'gobj_composite2.obj', 'gsgbase_composite1.obj',
          'gsgmisc_geomIssuer.obj', 'linmath_composite1.obj',
          'linmath_composite2.obj',
          'mathutil_composite1.obj', 'mathutil_composite2.obj', 
          'parametrics_composite1.obj', 'parametrics_composite2.obj',
          'pnmimagetypes_pnmFileTypePNG.obj', 'pnmimagetypes_pnmFileTypeTIFF.obj', 'pnmimagetypes_composite1.obj',
          'pnmimagetypes_composite2.obj', 'pnmimage_composite1.obj', 'pnmimage_composite2.obj',
          'text_composite1.obj', 'text_composite2.obj',
          'tform_composite1.obj', 'tform_composite2.obj',
          'lerp_composite1.obj',
          'putil_composite1.obj', 'putil_composite2.obj',
          'audio_composite1.obj', 'pgui_composite1.obj', 'pgui_composite2.obj',
          'pandabase_pandabase.obj', 'libpandaexpress.dll', 'libdtoolconfig.dll', 'libdtool.dll']
if RUNINTERROGATE:
    OBJFILES.append("libpanda_module.obj")    
    OBJFILES.append("librecorder_igate.obj")    
    OBJFILES.append('libdisplay_igate.obj')    
    OBJFILES.append('libdgraph_igate.obj')    
    OBJFILES.append('libevent_igate.obj')    
    OBJFILES.append('libgobj_igate.obj')    
    OBJFILES.append('libgrutil_igate.obj')    
    OBJFILES.append('libdevice_igate.obj')    
    OBJFILES.append('libcollide_igate.obj')    
    OBJFILES.append('libgsgbase_igate.obj')    
    OBJFILES.append('libmathutil_igate.obj')    
    OBJFILES.append('liblerp_igate.obj')    
    OBJFILES.append('libputil_igate.obj')    
    OBJFILES.append('libaudio_igate.obj')    
    OBJFILES.append('libpgui_igate.obj')    
    OBJFILES.append('libtform_igate.obj')    
    OBJFILES.append('libparametrics_igate.obj')    
    OBJFILES.append('liblinmath_igate.obj')    
    OBJFILES.append('libpstatclient_igate.obj')    
    OBJFILES.append('libchar_igate.obj')    
    OBJFILES.append('libchan_igate.obj')    
    OBJFILES.append('libpnmimage_igate.obj')    
    OBJFILES.append('libpgraph_igate.obj')   
    OBJFILES.append('libtext_igate.obj')   
if OMIT.count("HELIX")==0:
...
OBJECTS =      [ 'pandafx_pandafx.obj',
                 ...
                 'libdtool.dll' ]
if RUNINTERROGATE:
    OBJECTS.append( 'libdistort_igate.obj' )
    OBJECTS.append( 'libpandafx_module.obj' )
    OBJECTS.append( 'libeffects_igate.obj' )
CompileLink(dll='libpandafx.dll', opts=['ADVAPI', 'NSPR', 'NVIDIACG'], obj=OBJECTS )
...
if (sys.platform != "win32"):
    ...
    if RUNINTERROGATE:
       OBJECTS.append( 'libglxdisplay_igate.obj' )
    ...
    CompileLink(opts=['GLUT', 'NVIDIACG', 'CGGL', 'NSPR'], dll='libpandagl.dll', obj=OBJECTS )
...
if (sys.platform == "win32"):
    ...
    CompileLink(dll='libpandadx7.dll', opts=['ADVAPI', 'WINGDI', 'WINKERNEL', 'WINUSER', 'WINMM', 'DXDRAW', 'DXGUID', 'D3D9', 'NSPR'], obj=[
...
OBJECTS = [  'pandaegg_pandaegg.obj',
             ...
             'libdtool.dll' ]
if RUNINTERROGATE:
    OBJECTS.append( 'libpandaegg_module.obj' )
    OBJECTS.append( 'libegg2pg_igate.obj' )
    OBJECTS.append( 'libegg_igate.obj' )
CompileLink(dll='libpandaegg.dll', opts=['ADVAPI', 'NSPR'], obj=OBJECTS )
...
OBJECTS =  [ 'pandaphysics_pandaphysics.obj',
             ...
             'libdtool.dll' ]
if RUNINTERROGATE:
    OBJECTS.append( 'libpandaphysics_module.obj' )
    OBJECTS.append( 'libphysics_igate.obj' )
    OBJECTS.append( 'libparticlesystem_igate.obj' )
CompileLink(dll='libpandaphysics.dll', opts=['ADVAPI', 'NSPR'], obj= OBJECTS )
...
CompileC(ipath=IPATH, opts=OPTS, src='fractalSplineNode.cxx', obj='pview_fractalSplineNode.obj')
CompileC(ipath=IPATH, opts=OPTS, src='fractalSplineBoxNode.cxx', obj='pview_fractalSplineBoxNode.obj')
CompileC(ipath=IPATH, opts=OPTS, src='fractalSplineCylinderNode.cxx', obj='pview_fractalSplineCylinderNode.obj')
CompileC(ipath=IPATH, opts=OPTS, src='fractalSplinePrismNode.cxx', obj='pview_fractalSplinePrismNode.obj')
CompileLink(dll='pview.exe', opts=['ADVAPI', 'NSPR','FRACTALSPLINE'], obj=[
             'pview_pview.obj',
             'pview_fractalSplineNode.obj',
             'pview_fractalSplineBoxNode.obj',
             'pview_fractalSplinePrismNode.obj',
             'pview_fractalSplineCylinderNode.obj',
             'libframework.dll',
             'libpanda.dll',
             'libpandafx.dll',
             'libpandaexpress.dll',
...
OBJECTS =  [ 'direct_direct.obj',
             ...
             'libdtool.dll' ]
if RUNINTERROGATE:
    OBJECTS.append( 'libdirect_module.obj' ) 

    OBJECTS.append( 'libdcparser_igate.obj' ) 
    OBJECTS.append( 'libshowbase_igate.obj' ) 
    OBJECTS.append( 'libdeadrec_igate.obj' ) 
    OBJECTS.append( 'libinterval_igate.obj' ) 
    OBJECTS.append( 'libdistributed_igate.obj' ) 
CompileLink(dll='libdirect.dll', opts=['ADVAPI', 'NSPR', 'SSL'], obj=OBJECTS )
...
if os.path.exists( "models" ):
    ...
    print "Skipping models copy: models directory doesnt exist"
